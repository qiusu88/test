...
1.Introduction
   ...
   specification:  
   超文本传输协议（http）是一种无状态的应用层协议，使用可扩展语法和自描述信息载荷以便于基于网络的超文本信息系统进行灵活交互。这个文档是一系列HTTP/1.1规范文档的第一系列：  

   1.  "Message Syntax and Routing" (this document) 消息语法和路由  

   2.  "Semantics and Content" [RFC7231]  词义和内容  

   3.  "Conditional Requests" [RFC7232]  条件请求  

   4.  "Range Requests" [RFC7233]  范围请求  

   5.  "Caching" [RFC7234]  缓存  

   6.  "Authentication" [RFC7235]  鉴权  
   ...
   HTTP/1.1规范已经废弃了RFC 2616 和 RFC 2145（关于HTTP版本的）。本规范同样更新了RFC2817中定义的使用CONNECT建连通道的说明，以及定义了在RFC2818中非正式描述的“https” URI 协议。  
   ...
   HTTP对信息系统是一个通用接口协议。设计用来隐藏服务器如何提供一个统一的不依赖于提供的资源类型的接口给客户端的细节。同时，服务器不需要清楚每个客户端的目的：一个HTTP请求可以被认为是独立的，而不是和客户端的类型或者和应用步骤的先决系列有关联的。结果是协议可以在不同的环境中被高效的使用，并且随着时间的迁移应用可以独立的发展。
